...
__webpack_require__.d(tron_namespaceObject, {
  ...
  getAllTransactionSlice: () => (getAllTransactionSlice),
...
__webpack_require__.d(methods_namespaceObject, {
  ...
  fetchEstimateDiesel: () => (transactions_fetchEstimateDiesel),
  ...
  fetchTonActivityDetails: () => (fetchTonActivityDetails),
  ...
  tryUpdateSwapTokens: () => (tryUpdateSwapTokens),
...
async function fetchStoredAddress(accountId, chain) {
  return (await fetchStoredAccount(accountId))[chain].address;
}
async function fetchStoredTonWallet(accountId) {
  return (await fetchStoredTonAccount(accountId)).ton;
}
async function fetchStoredTronWallet(accountId) {
...
async function fetchStoredTonAccount(accountId) {
  const account = await fetchStoredAccount(accountId);
  if (account.ton) return account;
  throw new Error('TON wallet missing');
}
async function fetchStoredTronAccount(accountId) {
  const account = await fetchStoredAccount(accountId);
  if (account.tron) return account;
...
async function updateStoredTonWallet(accountId, partial) {
...
function getAddressesFromAccount(account) {
...
const tokensCache = {
  baseCurrency: config/* DEFAULT_PRICE_CURRENCY */.wOb,
  bySlug: {
...
async function updateTokens(tokens, onUpdate, tokenDetails, baseCurrency, shouldSendUpdate) {
  const tokensForDb = [];
...
function mergeSortedActivities(array1, array2, isAsc) {
  ...
  for (let i = 0; i < lists.length; i++) {
    if (!areActivitiesSorted(lists[i], isAsc)) {
      (0,logs/* logDebugError */.SJ)(`Activity list ${i} is not sorted`, {
        stack: new Error().stack
      });
      lists[i] = sortActivities([...lists[i]], isAsc);
    }
  }
  return (0,iteratees/* mergeSortedArrays */.yQ)(array1, array2, (a1, a2) => compareActivities(a1, a2, isAsc), true);
}
function mergeSortedActivityIds(ids1, ids2, activityById, isAsc) {
  ...
  for (let i = 0; i < lists.length; i++) {
    if (!areActivityIdsSorted(activityById, lists[i], isAsc)) {
      logDebugError(`Activity id list ${i} is not sorted`, {
        stack: new Error().stack
      });
      lists[i] = sortActivityIds(activityById, [...lists[i]], isAsc);
    }
  }
  return mergeSortedArrays(ids1, ids2, (id1, id2) => compareActivities(activityById[id1], activityById[id2], isAsc), true);
}
...
class FallbackPollingScheduler {
  ...
  #schedulePolling() {
    ...
    this.#cancelScheduledPoll = (0,schedulers/* setCancellableTimeout */.Gf)(pollingStartDelay, handleTimeout);
  }

  /** Sets up a polling running when the socket is connected (to mitigate network and backend problems) */
  #scheduleForcedPolling() {
    var _this$cancelScheduled3;
    (_this$cancelScheduled3 = this.#cancelScheduledPoll) === null || _this$cancelScheduled3 === void 0 || _this$cancelScheduled3.call(this);
    const schedule = () => {
      const {
        forcedPollingPeriod
      } = this.#options;
...
function buildWallet(publicKey, walletVersion) {
...
async function getAccountBalance(accountId) {
  ...
  const {
    address
  } = await fetchStoredTonWallet(accountId);
...
async function pickBestWallet(network, publicKey) {
  ...
  const withLastTx = (0,iteratees/* findLast */.Uk)(allWallets, _ref2 => {
    let {
      lastTxId
    } = _ref2;
    return !!lastTxId;
...
function getOtherVersionWallet(network, wallet, otherVersion) {
  if (!wallet.publicKey) {
...
class PrivateKeySigner {
  constructor(walletAddress, walletVersion) {
  ...
  async signData(timestamp, domain, payload) {
    const keyPair = await this.getKeyPair();
    if ('error' in keyPair) return keyPair;
    const signature = await signDataWithPrivateKey(this.walletAddress, timestamp, domain, payload, keyPair.secretKey);
    return signer_Buffer.from(signature);
  }
  async encryptComment(comment, recipientPublicKey) {
    const keyPair = await this.getKeyPair();
    if ('error' in keyPair) return keyPair;
    const encrypted = await encryptMessageComment(comment, keyPair.publicKey, recipientPublicKey, keyPair.secretKey, this.walletAddress);
...
class MnemonicSigner extends PrivateKeySigner {
  ...
  getKeyPair = (0,withCache/* default */.A)(async () => {
...
class MockSigner extends PrivateKeySigner {
  ...
  getKeyPair() {
    ...
    return {
      publicKey: publicKeyHex ? (0,utils/* hexToBytes */.aT)(publicKeyHex) : signer_Buffer.alloc(64),
      secretKey: signer_Buffer.alloc(64)
    };
  }
}
class LedgerSigner {
...
function signTransactionsWithKeyPair(transactions, walletVersion, keyPair) {
  const secretKey = signer_Buffer.from(keyPair.secretKey);
...
async function fetchActivitySlice(accountId, tokenSlug, toTimestamp, fromTimestamp, limit, shouldIncludeFrom) {
  const {
    network
  } = parseAccountId(accountId);
  const {
...
function calculateActivityDetails(activity, parsedTrace) {
  ...
  const action = actions.find(_ref => {
    let {
      action_id
    } = _ref;
...
function getToncoinAmountForTransfer(token, willClaimMintless) {
  let amount = 0n;
  let realAmount = 0n;
  if (token.slug === config/* TON_USDT_SLUG */.rUK) {
...
doOverrideLedgerLimit) {
  ...
  try {
    ...
    const {
      ...
      parsedPayloads
    } = await isTokenBalanceInsufficient(network, account.ton.address, messages);
    const wallet = getTonWallet(account.ton);
    const {
      seqno,
      balance
    ...
    if ('error' in signingResult) return signingResult;
    const emulation = applyFeeFactorToEmulationResult(await emulateTransactionWithFallback(network, wallet, signingResult.transaction, account.ton.isInitialized));
    const result = {
      emulation,
      parsedPayloads
    };
...
async function decryptComment(accountId, encryptedComment, fromAddress, password) {
  const account = await fetchStoredTonAccount(accountId);
  const signer = getSigner(accountId, account, password);
  return signer.decryptComment(transfer_Buffer.from(encryptedComment, 'base64'), fromAddress);
}
...
function buildNftTransferMessage(nft, fromAddress, toAddress, comment) {
  ...
  const payload = isNotcoinBurn ? buildNotcoinVoucherExchange(fromAddress, nft.address, nft.index) : buildNftTransferPayload(fromAddress, toAddress, comment);
  return {
    payload,
...
function buildNotcoinVoucherExchange(fromAddress, nftAddress, nftIndex) {
  ...
  return buildNftTransferPayload(fromAddress, toAddress, payload, config/* NOTCOIN_FORWARD_TON_AMOUNT */.APY);
}
function buildNftTransferPayload(fromAddress, toAddress, payload) {
  ...
  let builder = new core_dist.Builder().storeUint(constants/* NftOpCode */.H3.TransferOwnership, 32).storeUint((0,util/* generateQueryId */.N)(), 64).storeAddress(core_dist.Address.parse(toAddress)).storeAddress(core_dist.Address.parse(fromAddress)).storeBit(false) // null custom_payload
...
activities, slug, isToNow) {
  ...
  try {
    ...
    const swaps = await swapGetHistory(address, {
      ...
      toTimestamp: toTime,
      asset: slug ? getTokenBySlug(slug).tokenAddress ?? config/* TONCOIN */.Tu9.symbol : undefined,
      hashes,
      isCex: true
    });
    if (!swaps.length) {
...
async function enrichActivities(accountId, activities, tokenSlug, isToNow) {
  try {
    ...
    (0,logs/* logDebugError */.SJ)('enrichActivities', err);
  }
  return activities;
}
async function reloadIncompleteActivities(accountId, activities) {
  try {
    const {
      network
    } = parseAccountId(accountId);
...
async function tryReloadIncompleteActivities(network, address, activities, actionIdsToReload) {
  const actionIdBatches = (0,iteratees/* split */.lD)(actionIdsToReload, FETCH_ACTIONS_BATCH_SIZE);
...
const inactiveUpdateConcurrencyLimiter = (0,schedulers/* createTaskQueue */.JL)();
function setupActivePolling(accountId, account, onUpdate, onUpdatingStatusChange, newestActivityTimestamps) {
  ...
  const stopActivityPolling = setupActivityPolling(accountId, account.ton.address, newestActivityTimestamps, handleWalletUpdate, onUpdate, onUpdatingStatusChange.bind(undefined, 'activities'));
  ...
  return pollingLoop({
    ...
    async poll() {
      try {
        ...
        const publicKeyBytes = (0,utils/* hexToBytes */.aT)(publicKey);
        const versions = (accountType === 'ledger' ? config/* LEDGER_WALLET_VERSIONS */.iQn : config/* POPULAR_WALLET_VERSIONS */.eZ2).filter(value => value !== version);
        const versionInfos = await getWalletVersionInfos(network, publicKeyBytes, versions);
        const filteredVersions = versionInfos.map(_ref => {
          let {
            wallet,
            ...rest
          } = _ref;
...
;// ./src/api/chains/tron/transactions.ts
...
async function getTokenTransactionSlice(accountId, slug, toTimestamp, fromTimestamp, limit) {
  const {
    network
  } = parseAccountId(accountId);
  const {
    address
  } = await fetchStoredTronWallet(accountId);
  let activities;
...
async function getAllTransactionSlice(accountId, toTimestamp, limit) {
  const {
    network
  } = parseAccountId(accountId);
  const tokenSlugs = getTokenSlugs(network);
  const chunks = (await Promise.all([getTokenTransactionSlice(accountId, config/* TRX */.X7T.slug, toTimestamp, undefined, limit), ...tokenSlugs.map(slug => getTokenTransactionSlice(accountId, slug, toTimestamp, undefined, limit))])).filter(chunk => chunk.length);
  if (!chunks.length) {
    return [];
  }
  ...
  return mergeActivities(trxChunk, ...tokenChunks).filter(_ref => {
    let {
      timestamp
    } = _ref;
    return timestamp >= fromTimestamp;
...
function mergeActivities(trxTxs) {
  ...
  let result = [];
  for (var _len = arguments.length, tokenTxs = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
    tokenTxs[_key - 1] = arguments[_key];
  }
  for (const tokenTxList of tokenTxs) {
    result = mergeSortedActivities(result, tokenTxList
  ...
  return mergeSortedActivities(result, trxTxs.filter(trxTx => !isSeenTxId(trxTx.id) && (trxTx.kind !== 'transaction' || trxTx.toAddress)));
}
...
const polling_inactiveUpdateConcurrencyLimiter = (0,schedulers/* createTaskQueue */.JL)();
...
function polling_setupInactivePolling(accountId, account, onUpdate) {
  ...
  const walletPolling = new WalletPolling({
    ...
    network: parseAccountId(accountId).network,
    address,
    onUpdate: polling_inactiveUpdateConcurrencyLimiter.wrap(async () => {
...
async function loadInitialActivities(accountId, tokenSlugs, onUpdate) {
  ...
  const chunks = await Promise.all(tokenSlugs.map(async slug => {
    var _activities$;
    let activities = await getTokenTransactionSlice(accountId, slug, undefined, undefined, FIRST_TRANSACTIONS_LIMIT);
...
async function loadNewActivities(accountId, newestActivityTimestamps, tokenSlugs, onUpdate) {
  ...
  const chunks = await Promise.all(tokenSlugs.map(async slug => {
    ...
    const activities = await getTokenTransactionSlice(accountId, slug, undefined, newestActivityTimestamp, FIRST_TRANSACTIONS_LIMIT);
...
/* provided dependency */ var tron_transfer_Buffer = __webpack_require__(48287)["Buffer"];
...
async function transfer_checkTransactionDraft(options) {
  ...
  try {
    ...
    if (tokenAddress) {
      ...
      const size = 9 + 60 + tron_transfer_Buffer.from(transaction.raw_data_hex, 'hex').byteLength + SIGNATURE_SIZE;
...
function fetchTokenActivitySlice(accountId, limit, slug, toTimestamp) {
  return getChainBySlug(slug) === 'ton' ? transactions_ton.fetchActivitySlice(accountId, slug, toTimestamp, undefined, limit) : tron.getTokenTransactionSlice(accountId, slug, toTimestamp, undefined, limit);
}
async function fetchAllActivitySlice(accountId, limit, toTimestamp) {
  const account = await fetchStoredAccount(accountId);
  const [tonActivities, tronActivities] = await Promise.all(['ton' in account ? transactions_ton.fetchActivitySlice(accountId, undefined, toTimestamp, undefined, limit) : [], 'tron' in account ? tron.getAllTransactionSlice(accountId, toTimestamp, limit) : []]);
  return mergeSortedActivitiesToMaxTime(tonActivities, tronActivities);
}
function transactions_checkTransactionDraft(chain, options) {
  return chains[chain].checkTransactionDraft(options);
}
async function transactions_submitTransfer(chain, options) {
  ...
  if (withDiesel && chain === 'ton') {
    result = await transactions_ton.submitTransferWithDiesel({
      accountId,
      password,
      toAddress,
      amount,
...
function createLocalTransactions(accountId, chain, params) {
  ...
  const localTransactions = params.map((subParams, index) => {
    ...
    if (subParams.normalizedAddress) {
      ...
      normalizedAddress = transactions_ton.normalizeAddress(toAddress, network);
    } else {
...
function transactions_fetchEstimateDiesel(accountId, tokenAddress) {
  const chain = chains.ton;
  return chain.fetchEstimateDiesel(accountId, tokenAddress);
}
async function fetchTonActivityDetails(accountId, activity) {
  const result = await chains.ton.fetchActivityDetails(accountId, activity);
  return (result === null || result === void 0 ? void 0 : result.activity) ?? activity;
}
...
async function connect(request, message, id) {
  try {
    ...
    const items = [buildTonAddressReplyItem(accountId, account.ton)];
    if (proof) {
...
;// ./src/api/common/prices.ts


async function getBaseCurrency() {
  return (await storages_storage.getItem('baseCurrency')) ?? config/* DEFAULT_PRICE_CURRENCY */.wOb;
}
...
async function swapBuildTransfer(accountId, password, request) {
  ...
  try {
    await swap_ton.validateDexSwapTransfers(network, address, request, transferList);
...
async function swapCexSubmit(chain, transferOptions, swapId) {
  const result = await transactions_submitTransfer(chain, transferOptions, false);
...
function initPolling(_onUpdate) {
  ...
  void Promise.allSettled([tryUpdateKnownAddresses(), tryUpdateTokens(), !config/* IS_STAKING_DISABLED */.OuE && tryUpdateSwapTokens(), tryUpdateStakingCommonData()]).then(() => resolveDataPreloadPromise());
...
function setupCommonBackendPolling() {
  const stopFns = [pollingLoop({
    ...
    skipInitialPoll: true,
    async poll() {
      await Promise.all([tryUpdateKnownAddresses(), !config/* IS_STAKING_DISABLED */.OuE && tryUpdateStakingCommonData(), tryUpdateConfig(), tryUpdateSwapTokens()]);
    }
  }).stop];
  return () => {
...
async function tryUpdateTokens() {
  try {
    ...
    const nonBackendTokenDetails = nonBackendTokenAddresses.length ? await callBackendPost(`/assets?base=${baseCurrency}`, {
      assets: nonBackendTokenAddresses.slice(0, MAX_POST_TOKENS)
    }) : undefined;
    await updateTokens(tokens, polling_onUpdate, nonBackendTokenDetails, baseCurrency, true);
  } catch (err) {
  ...
  try {
    ...
    const tokens = assets.reduce((acc, asset) => {
      ...
      acc[asset.slug] = {
        ...
        price: ((_tokensCache$bySlug$a = tokensCache.bySlug[asset.slug]) === null || _tokensCache$bySlug$a === void 0 ? void 0 : _tokensCache$bySlug$a.price) ?? 0
      };
      return acc;
    }, {});
    polling_onUpdate({
...
async function setActivePollingAccount(accountId, newestActivityTimestamps) {
  ...
  if (accountId) {
    const account = await fetchStoredAccount(accountId);
    const stopPollingFns = [!config/* IS_CORE_WALLET */.TI6 && setupAccountConfigPolling(accountId, account).stop, ...Object.keys(chains).map(chain => chains[chain].setupActivePolling(accountId, account, polling_onUpdate, setUpdatingStatus.bind(undefined, accountId, chain), pickChainTimestamps(newestActivityTimestamps, chain)))];
...
function createInactiveAccountsPollingManager() {
  ...
  function startAccountPolling(accountId, account) {
    ...
    const stopFns = Object.values(chains).map(chain => chain.setupInactivePolling(accountId, account, polling_onUpdate));
...
async function importMnemonic(network, mnemonic, password, version) {
  ...
  try {
    ...
    if (isBip39Mnemonic) {
      const tronWallet = auth_tron.getWalletFromBip39Mnemonic(network, mnemonic);
      tonWallet = await auth_ton.getWalletFromBip39Mnemonic(network, mnemonic);
      account = {
        type: 'bip39',
        mnemonicEncrypted,
        tron: tronWallet,
...
async function importViewAccount(network, addressByChain) {
  try {
    ...
    if (addressByChain.ton) {
      const wallet = await auth_ton.getWalletFromAddress(network, addressByChain.ton);
      if ('error' in wallet) return {
        ...wallet,
        chain: 'ton'
      };
...
async function importNewWalletVersion(accountId, version) {
  ...
  const newAccount = {
    ...account,
    ton: auth_ton.getOtherVersionWallet(network, account.ton, version)
  };
  const accounts = await fetchStoredAccounts();
  const existingAccount = Object.entries(accounts).find(_ref => {
    var _account$ton;
    let [, account] = _ref;
    return ((_account$ton = account.ton) === null || _account$ton === void 0 ? void 0 : _account$ton.address) === newAccount.ton.address && account.type === newAccount.type;
  });
  if (existingAccount) {
    return {
      isNew: false,
...
async function nfts_checkNftOwnership(accountId, nftAddress) {
  const account = await fetchStoredAccount(accountId);
  return 'ton' in account && nfts_ton.checkNftOwnership(accountId, nftAddress);
}
...
async function tonConnect_signTransfers(accountId, messages) {
  ...
  const preparedMessages = messages.map(_ref => {
    ...
    return {
      ...
      hints: {
        tokenAddress: (payload === null || payload === void 0 ? void 0 : payload.type) === 'tokens:transfer' ? getTokenBySlug(payload.slug).tokenAddress : undefined
      }
    };
  });
...
const APP_COMMIT_HASH = (/* unused pure expression or super */ null && ("03658ae9f7caa469fa0be2af5b3d6100b73de790"));
...
/* harmony export */ });
/* unused harmony exports pickTruthy, orderBy, unique, cloneDeep, isLiteralObject, filterValues, uniqueByKey, swapKeysAndValues */
...
/******/ 		var __webpack_exports__ = __webpack_require__.O(undefined, [800,885], () => (__webpack_require__(21519)))
...
/******/ 			return "" + chunkId + "." + {"76":"5e7f890cf92c2520527e","800":"09881729ade743d90448","885":"432b868ff25c6c3a0feb","909":"a7bb8f6eb0374ee347af"}[chunkId] + ".js";
/******/ 		};
/******/ 	})();
/******/ 	
...
/******/ })()
;
//# sourceMappingURL=941.e2dcebb506a6ee4b3938.js.map