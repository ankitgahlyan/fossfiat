...
const ledgerTransport = new dist/* TonTransport */.vs(new WindowTransport());
...
async function signTonTransactionsWithLedger(network, wallet, tonTransactions, subwalletId) {
  ...
  try {
    ledgerTransactions = await Promise.all(tonTransactions.map(tonTransaction => tonTransactionToLedgerTransaction(network, wallet.version, tonTransaction, ledgerVersion, isBlindSigningEnabled, subwalletId)));
  } catch (err) {
...
async function getIsBlindSigningEnabled(ledgerVersion) {
  ...
  const {
    blindSigningEnabled
  } = await ledgerTransport.getSettings();
 ...
 */
async function tonTransactionToLedgerTransaction(network, walletVersion, tonTransaction, ledgerVersion, isBlindSigningEnabled, subwalletId) {
  const {
    authType = 'external',
    sendMode = 0,
    seqno,
 ...
 * `knownJetton` field.
 */
async function getPayload(network, toAddress, tonPayload, ledgerVersion, isBlindSigningEnabled) {
  let {
    tokenAddress
  ...
  if ((ledgerPayload === null || ledgerPayload === void 0 ? void 0 : ledgerPayload.type) === 'jetton-transfer' && doesSupport(ledgerVersion, VERSION_WITH_JETTON_ID)) {
    if (!tokenAddress) {
 ...
 * Exported for tests only.
 */
function tonPayloadToLedgerPayload(tonPayload, ledgerVersion) {
  if (!tonPayload) {
    return undefined;
  }
  let ledgerPayload;
  ...
  if (ledgerPayload && !doesSupport(ledgerVersion, VERSION_WITH_PAYLOAD[ledgerPayload.type])) {
    (0,logs/* logDebug */.MD)(`The ${ledgerPayload.type} payload type is not supported by Ledger TON v${ledgerVersion}`);
...
function doesSupport(ledgerVersion, featureVersion) {
  return compareVersions(ledgerVersion, featureVersion) >= 0;
}
function getKnownJetton(tokenAddress) {
  const jettonId = knownJettonAddresses[tokenAddress];
  // eslint-disable-next-line no-null/no-null
  return jettonId === undefined ? null : {
    jettonId,
    workchain: constants/* WORKCHAIN */.Zm
  };
}
function getWalletSpecifiers(walletVersion, ledgerVersion, subwalletId) {
...
//# sourceMappingURL=76.5e7f890cf92c2520527e.js.map